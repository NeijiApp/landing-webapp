...
export async function POST(request: NextRequest) {
	try {
		...
		const testSegments: Array<
			{ type: "text"; content: string } | { type: "pause"; duration: number }
		> = [
			{
				type: "text",
				content:
					"Welcome to your test meditation. Take a deep breath and relax.",
			},
			{
				type: "pause",
				duration: 1,
		...
		console.log(
			`🔧 Prepared ${testSegments.length} test segments for assembly`,
		);

		// Generate the minimal meditation
		console.log(
			`🎤 Using voice ID for test generation: ${voiceId} (${gender})`,
		);
