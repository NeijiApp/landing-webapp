...
import { z } from "zod";
import { ttsRouter, type TTSProvider } from "~/lib/meditation/tts-router";
...
const testProviderSchema = z.object({
  text: z.string().optional().default("Hello, this is a test of the TTS system."),
...
async function handleSwitchProvider(body: any) {
  try {
    ...
    console.log(`🔄 [TTS API] Switching to provider: ${provider.toUpperCase()}`);
...
async function handleTestProvider(body: any) {
  try {
    ...
    console.log(`🧪 [TTS API] Testing current provider with: "${text.substring(0, 30)}..."`);
