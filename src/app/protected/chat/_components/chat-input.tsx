...
export function ChatInput({ onChatFocus }: ChatInputProps) {
    ...
    const handleMeditationSubmitFromInput = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch("/api/meditation", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ prompt: currentInput, voiceId: defaultVoiceId, gender: "female", duration: 5, background: "silence", guidance: "confirmed", goal: "calm" }) });
            if (!response.ok) throw new Error(await response.text());
            const audioBlob = await response.blob();
            if (audioBlob.size === 0) throw new Error("Received empty audio file");
            const audioUrl = URL.createObjectURL(audioBlob);
    ...
    return (
        <>
            ...
            <div className={cn("fixed right-1/2 bottom-[92px] z-5 w-full max-w-xl translate-x-1/2 transition-all duration-300 ease-in-out", meditationMode === "meditation" ? (isExpanded ? "h-[min(70dvh,calc(100dvh-140px))]" : "h-[min(45dvh,calc(100dvh-140px))]") : "h-0")}> 
            ...
            <div className="fixed right-1/2 bottom-0 z-10 w-full max-w-xl translate-x-1/2 self-center">
                <div className="rounded-t-2xl bg-white/85 p-3 pb-[calc(12px+env(safe-area-inset-bottom))] shadow-lg backdrop-blur-md md:p-4">
                    <div className="flex items-center gap-3">
                        <Button onClick={() => { setMeditationMode(meditationMode === "meditation" ? "chat" : "meditation"); if (meditationMode === "meditation") setIsExpanded(false); }} size="icon" className="size-12 rounded-full">
                        ...
                        <form onSubmit={finalHandleSubmit} className="relative flex-1">
                            ...
                            <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                {isLoading ? (
                                    ...
                                    <Button type="submit" size="icon" className="size-9 rounded-full" disabled={!input.trim()}>
