/**
 * @fileoverview Page Chat - Interface de chat IA pour les utilisateurs authentifiés
 * 
 * Cette page fournit une interface de chat avancée permettant aux utilisateurs
 * authentifiés d'interagir avec l'IA Neiji. Elle inclut des fonctionnalités
 * de chat en temps réel, génération audio, et gestion des conversations.
 * 
 * @component ChatPage
 * @description Interface de chat IA avec fonctionnalités avancées
 * 
 * Fonctionnalités principales :
 * - Chat en temps réel avec l'IA Neiji
 * - Animation d'écriture en machine à écrire pour les réponses du bot
 * - Génération et lecture audio des réponses IA
 * - Gestion de l'historique des conversations
 * - Interface responsive et moderne
 * - Pas de limitation de messages (utilisateurs authentifiés)
 * - Pas de collecte d'email (utilisateurs déjà connectés)
 * 
 * @author Neiji Team
 * @version 1.0.0
 * @since 2025
...
export default function ChatPage() {
	...
	const handleStartChat = () => {
		...
		setTimeout(() => {
			const welcomeMessage: ChatMessage = {
				id: Date.now(),
				sender: "bot",
				text: "Hey ! What is the one thing you want to improve in your life today ?",
				avatar: ppBot,
				senderName: "Neiji",
				isTyping: true,
			};
	...
	const handleSendMessage = async () => {
		...
		try {
			...
			// Normal flow - show the bot response (no email collection for authenticated users)
			botId = Date.now() + 1;
			setMessages((prev) => [
				...prev,
				{
			...
			<div className="message-container sticky bottom-0 w-full max-w-xl self-center p-4 transition-all duration-500 ease-in-out">
				<div className="relative flex items-center gap-2">
					...
					<input
						...
						placeholder={chatStarted ? "Message" : "Start chatting with Neiji"}
