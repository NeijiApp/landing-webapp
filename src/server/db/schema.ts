...
export const meditationAnalytics = pgTable(
	...
	(table) => ({
		meditationIdIdx: index("idx_meditation_analytics_meditation_id").on(
			table.meditation_id,
		),
	}),
);
...
export const conversationHistory = pgTable(
	...
	(table) => ({
		...
		createdAtIdx: index("idx_conversation_history_created_at").on(
			table.created_at,
		),
	}),
);
...
export const audioSegmentsCache = pgTable(
	...
	(table) => ({
		...
		lastUsedIdx: index("idx_audio_segments_cache_last_used").on(
			table.lastUsedAt,
		),
		textHashIdx: index("idx_audio_segments_cache_text_hash").on(table.textHash),
		usageCountIdx: index("idx_audio_segments_cache_usage_count").on(
			table.usageCount,
		),
		voiceIdIdx: index("idx_audio_segments_cache_voice_id").on(table.voiceId),
		unique_segment: unique("audio_segments_cache_unique_segment").on(
			table.textHash,
			table.voiceId,
			table.voiceStyle,
		),
