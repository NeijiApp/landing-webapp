...
export function validateSegmentsCompatibility(segments: AudioSegment[]): {
	...
	segments.forEach((segment, index) => {
		...
		if (segment.duration <= 0) {
			errors.push(
				`Segment ${index + 1}: Durée invalide (${segment.duration}s)`,
			);
		}

		if (segment.duration > 1800) {
			// 30 minutes max par segment
			warnings.push(
				`Segment ${index + 1}: Durée très longue (${segment.duration}s)`,
			);
		}
		...
		if (segment.silenceAfter && segment.silenceAfter > 60) {
			warnings.push(
				`Segment ${index + 1}: Silence très long (${segment.silenceAfter}s)`,
			);
		}
	});
	...
	if (totalDuration > 3600) {
		// 1 heure max
		warnings.push(
			`Durée totale très longue: ${Math.round(totalDuration / 60)} minutes`,
		);
	}
 ...
 */
export function optimizeSegmentsForAssembly(
	...
	return segments.map((segment, index) => {
		...
		if (index > 0) {
			optimized.fadeIn = Math.min(
				optimized.fadeIn || 200,
				segment.duration * 0.1 * 1000,
			);
		}

		if (index < segments.length - 1) {
			optimized.fadeOut = Math.min(
				optimized.fadeOut || 200,
				segment.duration * 0.1 * 1000,
			);
		}
...
export function generateAssemblyFilename(
	...
	const goal = request.segments[0]?.id.includes("stress")
		? "stress"
		: request.segments[0]?.id.includes("sleep")
			? "sleep"
			: request.segments[0]?.id.includes("focus")
				? "focus"
				: "meditation";
