...
export class AssemblyClient {
	...
	async assembleAudio(request: AssemblyRequest): Promise<AssemblyResult> {
		...
		try {
			...
			this.log(
				"info",
				`Assembly completed in ${processingTime}ms for request ${request.requestId}`,
			);
			...
			this.log(
				"error",
				`Assembly failed after ${processingTime}ms for request ${request.requestId}: ${error}`,
			);
	...
	private async makeRequestWithRetries(
		...
		for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
			try {
				...
				this.log(
					"warn",
					`Request attempt ${attempt} failed, retrying in ${this.config.retryDelay}ms: ${error}`,
				);
...
export function getAssemblyClient(
	...
	if (!assemblyClientInstance) {
		throw new Error(
			"Assembly client not initialized. Please provide configuration.",
		);
	}
