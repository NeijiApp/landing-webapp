...
const AI_CONFIG = {
	...
	highSimilarityThreshold: 0.9, // 90%+ = réutilisation avec ajustement
...
export class MeditationAIAgent {
	 ...
	 */
	async generateOptimizedMeditation(
		...
		try {
			...
			console.log(
				`📋 Plan created: ${plan.segments.length} segments, optimization ${plan.optimizationScore.toFixed(1)}/5`,
			);
	...
	private async createGenerationPlan(
		...
		for (const segment of segments) {
			...
			console.log(
				`⚙️ Segment "${segment.type}": ${decision.action} (${(decision.confidence * 100).toFixed(1)}% confiance)`,
			);
		}

		// 3. Calculer le score d'optimisation global
		const reuseRate =
			decisions.filter((d) => d.action !== "create_new").length /
			decisions.length;
	 ...
	 */
	private async parseRequestToSegments(
		request: MeditationRequest,
	): Promise<SegmentPlan[]> {
	...
	private async makeOptimizationDecision(
		...
		if (
			cacheResult.recommendation === "use_similar" &&
			cacheResult.similar.length > 0
		) {
			const bestMatch = cacheResult.similar[0];
			if (
				bestMatch &&
				bestMatch.similarity >= AI_CONFIG.highSimilarityThreshold
			) {
				return {
	...
	private async executePlan(
			...
			try {
				...
				switch (decision.action) {
					...
					case "reuse_similar":
						...
						segmentsReused++;
						console.log(
							`🔄 Réutilisation similaire: ${segment.type} (${(decision.similarity! * 100).toFixed(1)}%)`,
						);
						break;
				...
				errors.push(
					`Erreur ${segment.type}: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
				);
			}
		}
		...
		return {
			...
			actualQuality: this.calculateActualQuality(
				segmentsReused,
				segmentsCreated,
			),
			segmentsReused,
	...
	private selectTemplate(goal: string, duration: number) {
		...
		return [
			...
			{ type: "visualization" as const, percentage: 0.2, priority: 2 },
	...
	private generateSegmentContent(
		...
		const baseContent = {
			...
			body_scan:
				"Focus your attention on your body, starting from the top of your head",
			visualization: "Imagine yourself in a peaceful and safe place",
			conclusion: "Gently return to the present moment",
		};

		return (
			baseContent[templateSegment.type as keyof typeof baseContent] ||
			"Meditation content"
		);
	}
	...
	private async createNewSegment(
		...
		console.log(
			`🎤 Création TTS pour: "${segment.content.substring(0, 50)}..."`,
		);
	...
	private async updateMetrics(result: GenerationResult): Promise<void> {
		...
		this.metrics.totalCostSaved +=
			result.optimizationAchieved * result.actualCost;
		this.metrics.reuseRate =
			(this.metrics.reuseRate * (this.metrics.totalRequests - 1) +
				result.optimizationAchieved) /
			this.metrics.totalRequests;
		this.metrics.averageQuality =
			(this.metrics.averageQuality * (this.metrics.totalRequests - 1) +
