...
import type {
	InsertAudioSegmentsCache,
	SelectAudioSegmentsCache,
...
export async function updateAudioSegmentInCache(
	...
	try {
		...
		if (updatedSegment) {
			console.log(
				`🔄 Updated existing cache entry: "${text.substring(0, 50)}..."`,
			);
		}
...
export async function saveAudioSegmentToCache(
	...
	if (process.env.VERCEL === "1") {
		console.log(
			"🚫 Sauvegarde de cache désactivée sur Vercel pour éviter les erreurs SASL",
		);
		return null;
	}
	...
	const existing = await updateAudioSegmentInCache(
		text,
		voiceId,
		voiceStyle,
		audioUrl,
	...
	try {
		...
		if (savedSegment) {
			...
			generateEmbedding(text)
				...
				.catch((error) => {
					console.warn(
						`⚠️ Échec génération embedding pour segment ${savedSegment.id}:`,
						error,
					);
				});
		}
...
export async function findBestCachedSegment(
	...
	if (process.env.VERCEL === "1") {
		console.log(
			"🚫 Recherche de cache complètement désactivée sur Vercel pour éviter les erreurs SASL",
		);
		return {
			exact: null,
			similar: [],
