...
import { initializeAudioBucket, saveAudioToStorage } from "./audio-storage";
...
export const generateParagraphAudioOpenAI = async (
	...
	// 1. Check cache first
	console.log(
		`🔍 [OpenAI TTS] Checking cache for: "${paragraph.substring(0, 50)}..."`,
	);
	...
	if (
		...
		console.log(
			`✅ [OpenAI TTS] Found cached audio segment (ID: ${cachedSegment.id})`,
		);
		...
		try {
			...
			if (cachedResponse.ok && cachedResponse.body) {
				console.log(
					`🎵 [OpenAI TTS] Serving cached audio from: ${cachedSegment.audioUrl}`,
				);
	...
	console.log(
		`🎙️ [OpenAI TTS] Generating new audio for: "${paragraph.substring(0, 50)}..."`,
	);

	// Select appropriate voice
	const openaiVoice =
		VOICE_MAPPING[voice_id as keyof typeof VOICE_MAPPING] ||
