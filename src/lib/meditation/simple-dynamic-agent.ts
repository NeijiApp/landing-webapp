...
const MeditationSegmentSchema = z.object({
    type: z.enum(['introduction', 'breathing', 'body_scan', 'visualization', 'mindfulness', 'transition', 'conclusion']),
    text: z.string(),
    duration_seconds: z.number(),
    pause_after_seconds: z.number(),
    emotional_tone: z.enum(['calm', 'peaceful', 'energizing', 'grounding', 'healing', 'focused']),
...
export class SimpleDynamicAgent {
    ...
    async generateMeditation(request: GenerationRequest): Promise<{
        ...
        for (const match of matchedSegments) {
            ...
            if (match.similarity >= this.similarityThreshold && match.cachedAudioUrl) {
                reusedCount++;
                console.log(`♻️ Reusing segment (${(match.similarity * 100).toFixed(1)}% match): ${match.segment.type}`);
            } else {
                generatedCount++;
                cost += 0.002; // TTS cost
        ...
        console.log(`✅ Meditation ready: ${reusedCount} reused, ${generatedCount} new segments`);
    ...
    private generateIntelligentScript(request: GenerationRequest): MeditationScript {
        ...
        for (const segment of structure) {
            const content = this.generateContextualContent(segment.type, goal, request);
            const pauseDuration = this.calculateIntelligentPause(segment.type, goal, request.duration);
            
            segments.push({
                type: segment.type as 'introduction' | 'breathing' | 'body_scan' | 'visualization' | 'mindfulness' | 'transition' | 'conclusion',
                text: content,
    ...
    private calculateSegmentStructure(duration: number, goal: string) {
        ...
        if (duration <= 2) {
            // Short meditation
            return [
                { type: 'introduction' as const, duration: Math.round(totalSeconds * 0.15) },
                { type: 'breathing' as const, duration: Math.round(totalSeconds * 0.35) },
                { type: 'body_scan' as const, duration: Math.round(totalSeconds * 0.25) },
                { type: 'visualization' as const, duration: Math.round(totalSeconds * 0.20) },
                { type: 'conclusion' as const, duration: Math.round(totalSeconds * 0.05) }
            ];
        } else if (duration <= 5) {
            // Medium meditation
            return [
                { type: 'introduction' as const, duration: Math.round(totalSeconds * 0.12) },
                { type: 'breathing' as const, duration: Math.round(totalSeconds * 0.28) },
                { type: 'body_scan' as const, duration: Math.round(totalSeconds * 0.30) },
                { type: 'visualization' as const, duration: Math.round(totalSeconds * 0.25) },
                { type: 'conclusion' as const, duration: Math.round(totalSeconds * 0.05) }
            ];
        } else {
            // Long meditation
            return [
                { type: 'introduction' as const, duration: Math.round(totalSeconds * 0.10) },
                { type: 'breathing' as const, duration: Math.round(totalSeconds * 0.25) },
                { type: 'body_scan' as const, duration: Math.round(totalSeconds * 0.30) },
                { type: 'mindfulness' as const, duration: Math.round(totalSeconds * 0.20) },
                { type: 'visualization' as const, duration: Math.round(totalSeconds * 0.12) },
                { type: 'conclusion' as const, duration: Math.round(totalSeconds * 0.03) }
            ];
        }
    }

    private generateContextualContent(type: string, goal: string, request: GenerationRequest): string {
        const library = CONTENT_LIBRARY[type as keyof typeof CONTENT_LIBRARY];
        if (!library) return `${type} content for ${goal}`;
        
        const goalContent = library[goal as keyof typeof library] || library['calm' as keyof typeof library];
        if (!goalContent || !Array.isArray(goalContent)) return `${type} content for ${goal}`;
        
        // Intelligent selection based on user prompt and request context
        const selectedContent = this.selectBestContent(goalContent, request.userPrompt, type);
        
        // Personalize based on duration
        return this.personalizeContent(selectedContent, request);
    }

    private selectBestContent(options: string[], userPrompt: string, type: string): string {
        ...
        if (prompt.includes('work') || prompt.includes('focus') || prompt.includes('concentrate')) {
        ...
        return options[Math.floor(Math.random() * options.length)] || options[0] || 'calm';
    }

    private personalizeContent(content: string, request: GenerationRequest): string {
        // Add duration awareness
        let personalized = content.replace('this meditation', `this ${request.duration} minute meditation`);
        
        // Add context from user prompt
        if (request.userPrompt.includes('work')) {
            personalized = personalized.replace('the day', 'your work day');
        }
        if (request.userPrompt.includes('night') || request.userPrompt.includes('sleep')) {
    ...
    private calculateIntelligentPause(type: string, goal: string, duration: number): number {
        const basePauses = {
            introduction: 2,
    ...
    private selectEmotionalTone(goal: string, type: string): 'calm' | 'peaceful' | 'energizing' | 'grounding' | 'healing' | 'focused' {
        const toneMap: Record<string, Record<string, any>> = {
            sleep: { introduction: 'calm', breathing: 'peaceful', body_scan: 'peaceful', visualization: 'peaceful', conclusion: 'peaceful' },
            focus: { introduction: 'focused', breathing: 'focused', body_scan: 'grounding', visualization: 'focused', conclusion: 'focused' },
            morning: { introduction: 'energizing', breathing: 'energizing', body_scan: 'grounding', visualization: 'focused', conclusion: 'energizing' },
            calm: { introduction: 'calm', breathing: 'peaceful', body_scan: 'healing', visualization: 'peaceful', conclusion: 'calm' }
        };
    ...
    private generateBenefits(goal: string): string[] {
        const benefitMap: Record<string, string[]> = {
            sleep: ['Deep relaxation', 'Improved sleep quality', 'Reduced nighttime anxiety'],
            focus: ['Enhanced concentration', 'Mental clarity', 'Reduced distractibility'],
            morning: ['Increased energy', 'Clear intention setting', 'Positive day preparation'],
            calm: ['Stress reduction', 'Emotional balance', 'Inner peace']
        };
        
        return benefitMap[goal] || ['Increased awareness', 'Reduced stress', 'Greater well-being'];
    }

    private async findSimilarSegments(segments: MeditationSegment[]): Promise<SegmentMatch[]> {
