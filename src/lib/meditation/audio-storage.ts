...
if (supabaseUrl && supabaseServiceKey) {
	...
	console.warn(
		"⚠️ Supabase configuration missing - audio storage will be unavailable",
	);
}
...
export async function initializeAudioBucket(): Promise<void> {
	if (!supabase) {
		console.warn(
			"⚠️ Supabase not configured - skipping audio bucket initialization",
		);
		return;
	}
...
export async function deleteAudioFromStorage(audioUrl: string): Promise<void> {
	if (!supabase) {
		console.warn(
			"⚠️ Supabase not configured - cannot delete audio from storage",
		);
		return;
	}
	...
	try {
		...
		if (filesToDelete.length > 0) {
			...
			if (deleteError) {
				...
				console.log(
					`✅ Cleanup completed: ${filesToDelete.length} files deleted`,
				);
			}
		} else {
...
function extractFileNameFromUrl(url: string): string | null {
	try {
		const urlParts = url.split(
			"/storage/v1/object/public/" + AUDIO_BUCKET + "/",
		);
