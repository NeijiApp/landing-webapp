...
function parseSentencesWithPauses(
	raw: string,
): Array<
...
export class ContextualMeditationAgent {
	...
	async generateMeditation(request: GenerationRequest): Promise<{
		...
		try {
			...
			console.log(
				`🎯 Detected: ${context.primary_issue} (${context.emotional_state}, ${context.intensity_level} intensity)`,
			);
			...
			console.log(
				`⚙️ Protocol: ${protocol.protocol_name} with ${protocol.phases.length} phases`,
			);

			// Step 3: Generate contextual content
			monitor.logStep(
				"content_generation",
				"generate_therapeutic_content",
				"started",
			);
			const content = await this.generateContent(context, protocol, request);
			monitor.logContentGeneration(protocol, content);
			console.log(
				`📜 Generated ${content.segments.length} contextual segments`,
			);
			...
			const matchedSegments = await this.findSimilarSegments(
				content.segments,
				request.voiceGender,
				request.voiceStyle || "calm",
			);
			monitor.logStep(
				"similarity_analysis",
				"check_existing_segments",
			...
			console.log(
				`✅ Contextual meditation ready: ${reusedCount} reused, ${generatedCount} new segments`,
			);
...
Generate specific, personalized content for each phase that directly addresses their ${context.primary_issue} with ${context.emotional_state} at ${context.intensity_level} intensity.`;
	...
	private createFallbackProtocol(
		context: ContextAnalysis,
		request: GenerationRequest,
	): MeditationProtocol {
	...
	private createFallbackContent(
		...
		const segments: GeneratedContent["segments"] = protocol.phases.map(
			(phase) => ({
				...
				duration_seconds: Math.floor(
					totalSeconds * phase.duration_percentage * 0.7,
	...
	private generateFallbackText(
		...
		const texts: Record<string, string> = {
			Grounding:
				"Take a moment to settle into this space. Notice your breath naturally flowing in and out. Allow yourself to simply be present with whatever you're experiencing right now.",
			Deepening:
				"As you continue to breathe, begin to release any tension you're holding. With each exhale, let go a little more. You are safe here in this moment.",
			Integration:
				"Feel the sense of calm that's available to you. This peace is always within you, ready to be accessed whenever you need it.",
			Closing:
				"Begin to gently return your awareness to your surroundings. Take this sense of calm with you as you open your eyes when you're ready.",
		};
