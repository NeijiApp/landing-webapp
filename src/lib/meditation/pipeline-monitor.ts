...
export class MeditationPipelineMonitor {
	...
	logStep(
		...
		if (status === "completed") {
			const startStep = this.steps.find(
				(s) =>
					s.step === step && s.substep === substep && s.status === "started",
			);
			if (startStep) {
			...
			"completed",
			{
				...
				avgSegmentLength:
					segments.length > 0
						? segments.reduce(
								(sum: number, seg: any) =>
									sum + (seg?.spoken_content?.length || 0),
								0,
							) / segments.length
						: 0,
			},
	...
	private printStep(step: PipelineStep) {
		const timestamp =
			new Date(Date.now() - performance.now() + (step.timestamp || 0))
				.toISOString()
				.split("T")[1]
				?.split(".")[0] || "00:00:00";
		...
		console.log(
			`${color}[${timestamp}] ${icon} ${stepName.toUpperCase()} ${step.status.toUpperCase()} ${duration}${resetColor}`,
		);
	...
	private printStepData(step: PipelineStep) {
		...
		switch (step.step) {
			case "context_analysis":
				...
				console.log(
					`${indent}😰 State: ${step.data.emotionalState} (${step.data.intensity})`,
				);
			...
			case "protocol_generation":
				...
				console.log(
					`${indent}⏱️ Duration: ${step.data.totalDuration}min with ${step.data.phasesCount} phases`,
				);
				break;
			...
			case "content_generation":
				...
				console.log(
					`${indent}📝 Generated ${step.data.segmentsCount} segments (avg ${Math.round(step.data.avgSegmentLength)} chars)`,
				);
				break;
			...
			case "similarity_check":
				...
				console.log(
					`${indent}🔍 Found ${step.data.resultsCount} similar segments, best: ${similarity}%`,
				);
				console.log(
					`${indent}🎯 Decision: ${step.data.decision} (${step.data.textLength} chars)`,
				);
				break;
			...
			case "ai_call":
				...
				console.log(
					`${indent}💰 Cost: $${step.data.cost.toFixed(4)} (${step.data.tokens} tokens)`,
				);
				console.log(
					`${indent}📏 Input: ${step.data.promptLength} chars → Output: ${step.data.responseLength} chars`,
				);
				break;
			...
			case "tts_call":
				...
				console.log(
					`${indent}⏱️ Duration: ${Math.round(step.data.duration / 1000)}s`,
				);
				if (!step.data.cached) {
	...
	private printSummary() {
		...
		console.log(
			`🤖 AI Calls: ${this.metrics.aiCalls} (${this.metrics.totalTokens} tokens)`,
		);
		console.log(`🎙️  TTS Calls: ${this.metrics.ttsCalls}`);
		console.log(
			`♻️  Cache Performance: ${this.metrics.cacheHits} hits, ${this.metrics.cacheMisses} misses`,
		);
		...
		console.log("\n🔄 PIPELINE BREAKDOWN:");
		const stepsByType = this.steps.reduce(
			(acc, step) => {
