...
export async function generateEmbedding(
	...
	try {
		...
		throw new Error(
			`Échec génération embedding: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
		);
	}
}
...
export async function generateEmbeddingsBatch(
	...
	try {
		...
		throw new Error(
			`Échec génération batch: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
		);
	}
}
...
export async function saveEmbeddingToDatabase(
	...
	try {
		...
		throw new Error(
			`Échec sauvegarde: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
		);
	}
}
 ...
 */
export async function getEmbeddingFromDatabase(
	segmentId: number,
...
export async function findSimilarSegmentsByEmbedding(
	...
	if (process.env.VERCEL === "1") {
		console.log(
			"🚫 Recherche sémantique désactivée sur Vercel pour éviter les erreurs SASL",
		);
		return [];
	}
	...
	try {
		console.log(
			`🔍 Recherche sémantique pour: "${queryText.substring(0, 50)}..."`,
		);
		...
		for (const segment of segments) {
			...
			try {
				const segmentEmbedding = JSON.parse(
					segment.embedding,
				) as EmbeddingVector;
		...
		console.log(
			`✅ Trouvé ${sortedResults.length} segments similaires (seuil: ${threshold})`,
		);
...
export async function updateMissingEmbeddings(
	...
	try {
		...
		for (let i = 0; i < segmentsToProcess.length; i += batchSize) {
			...
			try {
				console.log(
					`🔄 Traitement batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(segmentsToProcess.length / batchSize)}`,
				);
		...
		console.log(
			`✅ Mise à jour terminée: ${processed} traités, ${errors} erreurs`,
		);
		...
		return {
			...
			coverage:
				result.totalSegments > 0
					? (result.withEmbeddings / result.totalSegments) * 100
					: 0,
