import type { MeditationRequest, SegmentPlan } from "./ai-agent";
...
const DURATION_PATTERNS = {
	short: {
		min: 3,
		max: 7,
		keywords: ["court", "rapide", "bref", "quick", "short"],
	},
	medium: {
		min: 8,
		max: 15,
		keywords: ["moyen", "normal", "standard", "medium"],
	},
	long: { min: 16, max: 30, keywords: ["long", "profond", "extended", "deep"] },
	extended: {
		min: 31,
		max: 60,
		keywords: ["très long", "extended", "marathon"],
	},
} as const;
...
const MEDITATION_TEMPLATES = {
	stress: {
		segments: [
			{ type: "intro", percentage: 0.1, priority: 5, focus: "accueil_stress" },
			{
				type: "breathing",
				percentage: 0.35,
				priority: 5,
				focus: "respiration_apaisante",
			},
			{
				type: "body_scan",
				percentage: 0.3,
				priority: 4,
				focus: "relachement_tension",
			},
			{
				type: "visualization",
				percentage: 0.2,
		...
		segments: [
			{
				...
				percentage: 0.08,
				priority: 4,
				focus: "preparation_sommeil",
			},
			{
				type: "breathing",
				percentage: 0.25,
				priority: 5,
				focus: "respiration_lente",
			},
			{
				type: "body_scan",
				percentage: 0.4,
				priority: 5,
				focus: "relaxation_progressive",
			},
			{
				type: "visualization",
				percentage: 0.25,
				priority: 4,
				focus: "endormissement",
			},
			{
				type: "conclusion",
				percentage: 0.02,
				priority: 2,
				focus: "transition_sommeil",
			},
		],
		defaultDuration: 15,
		voiceStyle: "warm",
		...
		segments: [
			{
				...
				percentage: 0.12,
				priority: 5,
				focus: "preparation_concentration",
			},
			{
				type: "breathing",
				percentage: 0.3,
				priority: 5,
				focus: "respiration_focalisee",
			},
			{
				type: "body_scan",
				percentage: 0.2,
				priority: 3,
				focus: "ancrage_corporel",
			},
			{
				type: "visualization",
				percentage: 0.3,
				priority: 5,
				focus: "clarte_mentale",
			},
			{
				type: "conclusion",
				percentage: 0.08,
		...
		segments: [
			{
				...
				percentage: 0.15,
				priority: 5,
				focus: "accueil_douleur",
			},
			{
				type: "breathing",
				percentage: 0.25,
				priority: 4,
				focus: "respiration_therapeutique",
			},
			{
				type: "body_scan",
				percentage: 0.4,
				priority: 5,
				focus: "exploration_sensation",
			},
			{
				type: "visualization",
				percentage: 0.15,
				priority: 4,
				focus: "guerison_visualisation",
			},
			{
				type: "conclusion",
				percentage: 0.05,
				priority: 3,
				focus: "integration_apaisement",
			},
		],
		defaultDuration: 12,
		voiceStyle: "warm",
		...
		segments: [
			{
				...
				percentage: 0.12,
				priority: 5,
				focus: "accueil_emotion",
			},
			{
				type: "breathing",
				percentage: 0.28,
				priority: 5,
				focus: "respiration_equilibrante",
			},
			{
				type: "body_scan",
				percentage: 0.25,
				priority: 4,
				focus: "ressenti_corporel",
			},
			{
				type: "visualization",
				percentage: 0.25,
				priority: 4,
				focus: "transformation_emotion",
			},
			{
				type: "conclusion",
				percentage: 0.1,
				priority: 4,
				focus: "integration_equilibre",
			},
		],
		defaultDuration: 10,
		voiceStyle: "warm",
		...
		segments: [
			{
				...
				percentage: 0.15,
				priority: 4,
				focus: "accueil_general",
			},
			{
				type: "breathing",
				percentage: 0.25,
				priority: 4,
				focus: "respiration_consciente",
			},
			{
				type: "body_scan",
				percentage: 0.3,
				priority: 4,
				focus: "presence_corporelle",
			},
			{
				type: "visualization",
				percentage: 0.25,
				priority: 3,
				focus: "bien_etre_general",
			},
			{
				type: "conclusion",
				percentage: 0.05,
				priority: 3,
				focus: "retour_present",
			},
		],
		defaultDuration: 10,
		voiceStyle: "calm",
...
export class MeditationParser {
	...
	static parseUserRequest(prompt: string, userId?: number): MeditationRequest {
		...
		console.log(
			`🔍 Analyse: Objectif="${goal}", Durée=${duration}min, Voix=${voicePreferences.gender}/${voicePreferences.style}`,
		);
	...
	static createDetailedSegments(request: MeditationRequest): SegmentPlan[] {
		const template =
			MEDITATION_TEMPLATES[request.goal as keyof typeof MEDITATION_TEMPLATES] ||
			MEDITATION_TEMPLATES.general;
	...
	private static detectDuration(prompt: string, goal: string): number {
		...
		if (durationMatch && durationMatch[1]) {
			const duration = Number.parseInt(durationMatch[1]);
		...
		const template =
			MEDITATION_TEMPLATES[goal as keyof typeof MEDITATION_TEMPLATES];
		return template?.defaultDuration || 10;
	}

	private static detectVoicePreferences(
		prompt: string,
		goal: string,
	): { gender: "male" | "female"; style: string } {
		...
		if (
			template &&
			!VOICE_PATTERNS.style[detectedStyle as keyof typeof VOICE_PATTERNS.style]
		) {
