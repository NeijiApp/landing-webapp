...
export const audioSegmentsCache = pgTable(
	...
	(table) => [
		index("idx_audio_segments_cache_language").using(
			"btree",
			table.language.asc().nullsLast().op("text_ops"),
		),
		index("idx_audio_segments_cache_last_used").using(
			"btree",
			table.lastUsedAt.asc().nullsLast().op("timestamp_ops"),
		),
		index("idx_audio_segments_cache_text_hash").using(
			"btree",
			table.textHash.asc().nullsLast().op("text_ops"),
		),
		index("idx_audio_segments_cache_usage_count").using(
			"btree",
			table.usageCount.asc().nullsLast().op("int4_ops"),
		),
		index("idx_audio_segments_cache_voice_id").using(
			"btree",
			table.voiceId.asc().nullsLast().op("text_ops"),
		),
		unique("audio_segments_cache_unique_segment").on(
			table.textHash,
			table.voiceId,
			table.voiceStyle,
		),
...
export const meditationHistory = pgTable(
	"meditation_history",
	{
		id: serial().primaryKey().notNull(),
	...
	(table) => [
		index("idx_meditation_history_user_id").using(
			"btree",
...
export const meditationFavorites = pgTable(
	...
	(table) => [
		index("idx_meditation_favorites_user_id").using(
			"btree",
			table.userId.asc().nullsLast().op("int4_ops"),
		),
		foreignKey({
		...
		unique("meditation_favorites_user_id_meditation_id_unique").on(
			table.userId,
			table.meditationId,
		),
	],
...
export const meditationAnalytics = pgTable(
	...
	(table) => [
		index("idx_meditation_analytics_meditation_id").using(
			"btree",
			table.meditationId.asc().nullsLast().op("int4_ops"),
		),
		foreignKey({
			columns: [table.meditationId],
			foreignColumns: [meditationHistory.id],
...
export const conversationHistory = pgTable(
	...
	(table) => [
		index("idx_conversation_history_created_at").using(
			"btree",
			table.createdAt.asc().nullsLast().op("timestamp_ops"),
		),
		index("idx_conversation_history_user_id").using(
			"btree",
			table.userId.asc().nullsLast().op("int4_ops"),
		),
		foreignKey({
